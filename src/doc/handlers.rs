...
use sqlx::PgPool;
use crate::doc::models::{Document, CreateDocument, UpdateDocument, Role};
...
pub async fn share_doc(
    ...
    let owner = sqlx::query_as::<_, crate::doc::models::Owner>(
        r#"
        SELECT owner_id
        FROM documents
        WHERE id = $1
    ...
    let existing = sqlx::query_as::<_, crate::doc::models::Collaborator>(
        r#"
    ...
    // Add collaborator
    let role: Role = payload.role.parse().map_err(|e: String| e)?;

    let collaborator = sqlx::query_as::<_, crate::doc::models::Collaborator>(
        r#"
